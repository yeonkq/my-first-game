...
try:
    b_img_path = get_path("dungeon.png")
    if os.path.exists(b_img_path):
        background_img = pygame.image.load(b_img_path).convert()
    ...
    for k, n in zip(keys, names):
        ...
        if os.path.exists(m_path):
            ...
            mob_images[k] = pygame.transform.scale(img, (BRICK_W, BRICK_H))
    ...
    for dx, dy in offsets:
        ...
        if SIDE_MARGIN <= new_rect.x <= WIDTH - BRICK_W and 40 <= new_rect.y < DEADLINE_Y:
            if is_space_empty(new_rect):
                bricks.append({"rect": new_rect, "hp": max(1, int(parent_brick["hp"] * 0.5)), "type": "clone"})
                spawned += 1
...
def launch_fireball(target_pos):
    ...
    projectiles.append({
        "pos": start_pos, "vel": [(dx/dist) * FIREBALL_SPEED, (dy/dist) * FIREBALL_SPEED],
...
while running:
    ...
    if game_state == "START":
        ...
        pygame.draw.rect(display_surface, (40, 40, 50), skill_rect, border_radius=8) # 배경 박스
        pygame.draw.rect(display_surface, (100, 100, 110), skill_rect, 2, border_radius=8) # 테두리
        
        if skill_slots["FIRE"]:
            # 활성화 상태 (빨간색 아이콘)
            inner_rect = skill_rect.inflate(-12, -12)
            pygame.draw.rect(display_surface, (230, 50, 50), inner_rect, border_radius=4)
            key_txt = small_font.render("1", True, (255, 255, 255))
            display_surface.blit(key_txt, (skill_rect.x + 5, skill_rect.y + 2))
        else:
            # 쿨타임 상태 (게이지 및 남은 마리 수)
            count = SKILL_THRESHOLD - bricks_destroyed
            count_txt = font.render(str(count), True, (150, 150, 160))
